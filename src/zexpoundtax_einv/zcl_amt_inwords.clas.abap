CLASS zcl_amt_inwords DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE .

  PUBLIC SECTION.
    CLASS-METHODS num2words IMPORTING iv_num          TYPE string OPTIONAL
                            CHANGING  iv_level        TYPE i OPTIONAL
                            RETURNING VALUE(rv_words) TYPE string  .
    CLASS-METHODS amount_dollar  IMPORTING iv_num          TYPE string OPTIONAL
...
CLASS ZCL_AMT_INWORDS IMPLEMENTATION.
  ...
  METHOD amount_dollar.
    ...
    ls_h-word2 = 'Hundred and' ##NO_TEXT .
...
*       This can be handled dynamically as well based on the requirement
        IF lv_dec_words IS NOT INITIAL .
          rv_words = |{ rv_words } { lv_dec_words } { iv_decsuffix } Only |  ##NO_TEXT .
        ELSE.
          rv_words = |{ rv_words } Only | ##NO_TEXT .
        ENDIF.
        CONDENSE rv_words.
...
*       It must be ensured that relative fractional monetary unit
*       shall be updated later in the code relative to the base unit
        rv_words = |{ rv_words } Rupees|  ##NO_TEXT .
        IF lv_dec <= 20.
          READ TABLE lt_d REFERENCE INTO DATA(ls_d2) WITH KEY num = lv_dec.
          IF sy-subrc = 0.
            lv_dec_words = |{ ls_d2->word }|.
...
*       has been used as fractional monetary unit in the code below
*       This can be handled dynamically as well based on the requirement
        rv_words = |{ rv_words } { lv_dec_words } Paise Only |  ##NO_TEXT .
        CONDENSE rv_words.
      ENDIF.
      RETURN.
    ENDIF.
